...
class ListapreciosController extends Controller
{
	...
	{
		$id = $_POST['ListaPrecios']['id_lista_precio'];
		$model=$this->loadModel($id);
		if(empty($model)){ 
		// Si el model esta vacio, quiere decir que es un insert nuevo. en caso contrario update

			$model=new ListaPrecios;
			if(isset($_POST['ListaPrecios']) )
			{
				$model->attributes=$_POST['ListaPrecios'];
				$model->listaprecio_fecha_alta= date('Y-m-d H:i:s');
				$model->listaprecio_estatus=1;
				if($model->save())
				{
	...
	{	$this->layout="main"; // Utilizamos el layout para administracion
		...
		$lista= ListaPrecios::model()->findAll();
	...
	{
		$model=ListaPrecios::model()->findByPk($id);
		return $model;
	}

	/**
	 * Performs the AJAX validation.
	 * @param Listaprecios $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='Listaprecios-form')
		{
	...
	public function actionListapreciosdatos()
	{
		$id = $_POST['id'];
		$Datos=ListaPrecios::model()->findBypk($id);
