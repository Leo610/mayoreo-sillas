...
class UnidadesdemedidaController extends Controller
{
	...
	{
		$id = $_POST['Unidadesdemedida']['id_unidades_medida'];
		$model=$this->loadModel($id);
		if(empty($model)){ 
		// Si el model esta vacio, quiere decir que es un insert nuevo. en caso contrario update

			$model=new Unidadesdemedida;
			if(isset($_POST['Unidadesdemedida']) )
			{
				$model->attributes=$_POST['Unidadesdemedida'];
				if($model->save())
				{
	...
	{	$this->layout="main"; // Utilizamos el layout para administracion
		...
		$lista= Unidadesdemedida::model()->findAll();
	 ...
	 * @return Unidadesdemedida the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Unidadesdemedida::model()->findByPk($id);
		return $model;
	}

	/**
	 * Performs the AJAX validation.
	 * @param Unidadesdemedida $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='Unidadesdemedida-form')
		{
	...
	{
		$id = $_POST['id'];
		$Datos=Unidadesdemedida::model()->findBypk($id);
