...
class Tipo_proyectosController extends Controller
{
	...
	{
		$id = $_POST['TipoProyectos']['id_tipo_proyecto'];
		$model=new TipoProyectos;
		...
		// Si el model esta vacio, quiere decir que es un insert nuevo. en caso contrario update
			...
			$model=new TipoProyectos;
			if(isset($_POST['TipoProyectos']) )
			{
				$model->attributes=$_POST['TipoProyectos'];
				if($model->save())
				{
	...
	{	$this->layout="main"; // Utilizamos el layout para administracion
		...
		$lista= TipoProyectos::model()->findAll();
	 ...
	 * @return Tipo_proyectos the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=TipoProyectos::model()->findByPk($id);
		return $model;
	}

	/**
	 * Performs the AJAX validation.
	 * @param Tipo_proyectos $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='Tipo_proyectos-form')
		{
	...
	{
		$id = $_POST['id'];
		$Datos=TipoProyectos::model()->findBypk($id);
